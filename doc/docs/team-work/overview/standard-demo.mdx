...
:::tip
其中，初次上手，最重要的例子是`jimer-sql`，按照你的语言喜好，打开这两个例子中任何一个即可
...
你也可以让它们用缓存模式启动，只需要将spring-boot的profile设置为`maxwell`或`debezium`即可。

这两个spring profile利用了[maxwell](https://maxwells-daemon.io/)或[debezium](https://debezium.io/)的CDC推送让能力实现<ViewMore buttonText="缓存一致性" variant="text"><CacheConsistencyPanel/></ViewMore>，因此，需要安装外部环境。

:::tip
事实上，借助[maxwell](https://maxwells-daemon.io/)或[debezium](https://debezium.io/)这样的CDC推送服务并非实现<ViewMore buttonText="缓存一致性" variant="text"><CacheConsistencyPanel/></ViewMore>的唯一途径，即使不借助于任何外部技术帮助，Jimmer也可以实现这个能力。
...
因此，虽然`Transaction触发器`可以在不依赖于任何特殊外部环境的情况下实现<ViewMore buttonText="缓存一致性" variant="text"><CacheConsistencyPanel/></ViewMore>，
...
2.  安装外部环境
    ...
    附带例子支持两种外部环境，你可以随意选择一种方式体验<ViewMore buttonText="缓存一致性" variant="text"><CacheConsistencyPanel/></ViewMore>
...
|debezium-demo-connect|5300|负责把MySQL的变化事件推送给Kafka的kafka-connector，可以通过http://localhost:5300/connectors来检验名为`debezium-connector`的pg-connector是否成功安装|
