...
-   Allow developers to use the intuitive method of directly modifying mutable objects to complete the "copy construction" of immutable objects. This is espeically suitable for deep-level immutable objects.

-   Reason: The java record, or the kotlin data class with `copy` function, is designed for simple immutable objects. it is difficult to handle complex deep-level immutable objects in a concise manner. Therefore, a sufficiently powerful solution is required to be ported.

:::note
This framework is named Jimmer, paying homage to [immer](https://github.com/immerjs/immer).
:::
