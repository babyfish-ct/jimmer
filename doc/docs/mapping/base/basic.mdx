...
By default, the column name of the id column is deduced from the Java property name. The default column names are all uppercase letters. The places where the original name switches from lowercase to uppercase are automatically separated by underscores. Here the corresponding database column name is ID.  
