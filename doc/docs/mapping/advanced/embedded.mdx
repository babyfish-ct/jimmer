...
Embedded fields merge multiple columns in the database into an integral whole, map them to a non-entity custom type, and then use this custom property to declare a property for the entity.
...
`org.babyfish.jimmer.sql.PropOverride` can be used to override column names of composite type fields:
