...
This is a classic `order - order detail - product` association.
...
Fortunately, for the classic `order - order detail - product` association, it is more intuitive to think of `order_item` as an independent entity, with two many-to-one associations pointing to `order_` and `product` respectively;
Rather than viewing `order-item` as an intermediate table and thinking there is a many-to-many relationship between `order_` and `product`.
