...
:::caution
Only when parameters are specified for the `@Transient` annotation will the current property be a complex formula property.
:::

So what are the parameters of the `@Transient` annotation?

Jimmer provides an interface for complex formula properties:
...
## Scalar Formula: `BookStore.avgPrice` 

In this section, we will add a formula property `BookStore.avgPrice` to `BookStore` with the type `java.math.BigDecimal`.

### Define Resolver for `avgPrice`

Each complex formula property corresponds to a `TransientResolver` implementation class.

Before defining the formula property `BookStore.avgPrice`, let's first define `BookStoreAvgPriceResolver`:
...
- The base interface `TransientResolver/KTransientResolver` has two generic parameters:
    ...
    -   The 1st generic parameter: The type of the id property of the entity to which the formula property belongs. 
    ...
    -   The 2nd generic parameter: The return data type of the formula property.  
...
-   `resolve` is a method of the base interface that must be implemented for users to complete calculation through it.
    ...
    This is designed to prevent the [N+1 problem](https://stackoverflow.com/questions/97197/what-is-the-n1-selects-problem-in-orm-object-relational-mapping) caused by formula properties.
...
-   `getDefaultValue` is an optional method of the base interface to implement.
    ...
    However, if the formula property (`BookStore.avgPrice` in this example) is non-null, it will cause problems. Users can override `getDefaultValue()` to return a non-null default value to resolve this issue.

    :::caution
    For `TransientResolver` implementations of formula properties that do not allow null:
...
-   ❸ Calculate the average of book prices within each group.
    ...
    -   `.asTableEx()`:
        ...
        By default, top-level queries do not allow JOIN through collection associations (one-to-many or many-to-many).
        
        This `asTableEx()` indicates that the developer is aware of what they are doing and needs such JOIN.  

        :::info
        Please refer to [Pagination Safety](../../../query/dynamic-join/table-ex) for more details.
        :::

    -   `.books()`:  
    
        From the current object `BookStore` as the starting point, associate to `Book` objects through the one-to-many associative property `BookStore.books`.

    -   `avg`: 

        Calculate the average of `Book.price` within the group.
...
Now that the `BookStoreAvgPriceResolver` class is complete, we can add the formula property `avgPrice` to the `BookStore` entity:
...
2.  Define the formula property `BookStore.avgPrice`, and specify the class introduced at ❶ for its `@Transient` annotation to tell Jimmer the calculation rule for the formula property.
...
## Associative Formula: `BookStore.newestBooks`

### Clarify Requirements

In the previous section we demonstrated the formula property `BookStore.avgPrice`, which is obviously a non-associative property.

In this section, we will add a formula property `BookStore.newestBooks` to `BookStore` with the type `java.util.List<Book>`, which is clearly an associative property.

To explain why this example needs to add a formula property `BookStore.newestBooks`, let's first look at the characteristics of the original associative property `BookStore.books`:
...
For example, there are a total of three books named "Effective TypeScript", with only different `edition` values: 3, 2, 1.

Now we want to create a new formula property `BookStore.newestBooks` through calculation. It ensures that the returned book collection has no duplicate name problem. For each unique book name, only the book with the highest `edition`, i.e. the maximum `edition`, is taken.

### Define Resolver for `newestBooks` 

Each complex formula property corresponds to a `TransientResolver` implementation class.

Before defining the formula property `BookStore.newestBooks`, let's first define `BookStoreNewestBooksResolver`:
...
-   ❸ `getDefaultValue` is an optional method of the base interface to implement.
    ...
    However, if the formula property (`BookStore.newestBooks` in this example) is non-null, it will cause problems. Users can override `getDefaultValue()` to return a non-null default value to resolve this issue.

    :::caution
    If formula properties do not allow null, for their `TransientResolver` implementations:
...
Now that the `BookStoreNewestBooksResolver` class is complete, we can add the formula property `newestBooks` to the `BookStore` entity:
...
2.  Define the formula property `BookStore.newestBooks`, and specify the class introduced at ❶ for its `@Transient` annotation to tell Jimmer the calculation rule for the formula property.
...
1.  Fetch the formula property `BookStore.newestBooks`

2.  The formula property itself is also an associative property, so the shape of its associated objects can be controlled by a deeper child fetcher.
...
:::tip
This example shows that when the formula property itself is also an associative property, the shape of its associated objects can be controlled by a deeper child fetcher.

Since there is a deeper child fetcher, of course both native ORM associative properties and other formula associative properties can be included.

That is, in the process of querying complex data structures by object fetchers, the fetching tasks of native ORM associative properties and formula associative properties can be arbitrarily mixed.

-   The fetching task of native ORM properties is actually SQL operations.

    (At least before we introduce caching, it can be considered so)
  
-   As mentioned earlier, Jimmer does not impose any restrictions on the calculation methods of formula properties. You can even use any technology other than SQL, such as OLAP technologies, to implement the calculation process (this documentation focuses on Jimmer itself, so examples also use Jimmer to implement the calculation process).

    That is, the fetching task of formula properties is not necessarily SQL operations.
