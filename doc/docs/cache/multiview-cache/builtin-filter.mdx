...
The `@LogicalDeleted` uses the built-in global filter to implement the soft delete logic. 

-   If `useMultiViewCache` of `@LogicalDeleted` annotation is false, 
    the corresponding built-in filter only implements `Filter/KFilter` interfaces, which is unfriendly to caching. 
    
-   If `useMultiViewCache` of `@LogicalDeleted` annotation is true, 
    the corresponding built-in filter only implements `CacheableFilter/KCacheableFilter` interfaces, which is caching-friendly. 
    
The default value of this property is `false`, indicating that all properties sensitive to this filter 
*(`BookStore.books`, `Author.books`, `BookStore.avgPrice`, and `BookStore.newestBooks`)* do NOT support caching, 
which is a more conservative option. 

To make all these properties cacheable, `useMultiViewCache = true` must be configured. 
:::

## Enable multi-view caching

How to enable multi-view caching has already been discussed in detail in the [previous document](./user-filter), 
...
However, unfortunately, due to the introduction of user-defined calculation rules that the framework cannot understand, developers must manually specify `SubKey` for the `Resolver` implementation of calculated properties.
...
-   Step 1: Query the aggregate root
    ...
    Even if cache them at the cost of sacrificing consistency is required, it should be handled by users as a business problem rather than an behavior that should be abstracted and unified by the framework.
    :::
