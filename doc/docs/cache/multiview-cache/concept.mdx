...
:::caution
Unlike previous discussions where cache can be categorized into [object cache](../cache-type/object), [association cache](../cache-type/association) and [computed cache](../cache-type/calculation), 
multi-view cache is not adding a new category to the existing taxonomy, but rather another dimension of characteristics.
:::

Except for [object cache](../cache-type/object), both [association cache](../cache-type/association) and [computed cache](../cache-type/calculation) can be combined with multi-view cache.
...
|Computed Cache|Single-view Computed Cache|Multi-view Computed Cache|

:::info
We can refer to association cache and computed cache together as property cache.
:::
...
As long as an entity is applied with a [global filter](../../query/global-filter), all association caches targeting it and computed caches relying on these association caches need to be multi-viewed.
...
:::caution
These filter-sensitive properties either do not support caching or support multi-view caching. Configuring single-view cache for them will be considered invalid and the cache configuration will be ignored.

Don't worry. Jimmer will tell developers [why caching is disabled](./abandoned-callback).
:::
...
Here we list examples of [object cache](../cache-type/object), [association cache](../cache-type/association) and [computed cache](../cache-type/calculation).
...
-   Key
    ...
    The Key in multi-view cache is no different from the Key in single-view cache, representing a specific object or a specific object property, decided by Jimmer.
...
Multi-view cache is only for property cache, i.e. association cache and computed cache. So association properties and complex computed properties must specify SubKey for their multi-view caches.
...
:::info
When multiple `CacheableFilter/KCachableFilter` affect the associated entity, the data returned by the `getParameters()` method of each filter object is merged together as the SubKey.
:::

### SubKey of Complex Computed Properties

Implementing [complex computed properties](../../mapping/advanced/calculated/transient) requires implementing the `TransientResolver/KTransientResolver` interface.
...
-   If `Ref` itself is null, it means getting SubKey is difficult and multi-view cache cannot be applied to the computed property.

    :::info
    In this case, Jimmer will tell developers [why caching is disabled](./abandoned-callback).
    :::

-   Otherwise, the internal value of `Ref` represents the SubKey of the current computed property. 
