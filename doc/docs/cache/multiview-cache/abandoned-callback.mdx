...
2.  The filtering rules for association properties and the calculation rules for calculated properties must be cache friendly.
    ...
    -   For association properties, **all** global filters that affect their associated objects must be cache friendly filters. If any one filter is not cache friendly, it may cause the property to be unable to use cache.
