...
:::caution
Unlike the fully automatic cache consistency of object cache and association cache, maintaining the consistency of calculated cache requires user assistance.
...
-   ❶ The calculated cache `BookStore.avgPrice-1` of the parent object referenced by the old value `1` of the foreign key is deleted.

-   ❷ The calculated cache `BookStore.avgPrice-2` of the parent object referenced by the new value `2` of the foreign key is deleted.
