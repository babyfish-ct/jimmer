...
-   The referenced association property is based on a pseudo foreign key
    ...
    The pseudo foreign key field may contain illegal values. Being non-null does not mean the associated object exists, so association cache is required to filter out valid associated objects.
...
:::info
The generated SQL would differ if cache is not enabled:

<ViewMore buttonText="Difference without Cache">

<Divider><Chip label="Cache Mode"/></Divider>
...
<Divider><Chip label="No Cache Mode"/></Divider>
