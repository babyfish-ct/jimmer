...
:::info  
If this threshold is not set, the default is `Integer.MAX_VALUE`, meaning no pagination query can enable this feature. 
:::
...
As you can see, when offset is too large, Jimmer queries page ids first, then converts the paged id set to object set. This is useful when data volume is large.
