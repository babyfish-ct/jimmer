...
1.  Table joins used by the where clause cannot be optimized.
    ...
    That is, **only** table joins used solely by the `select` or `order by` clause of the top-level query in the original data-query can be optimized.
...
3.  Table joins based on reference associations (one-to-one, many-to-one) may be optimized, if any of the following conditions is met:
    ...
    - Although join type is inner join, the association is based on non-null foreign key.
...
<table>
    ...
    <tr>
        <td colspan="2">
            Is used <b>solely</b> by select or orderBy clause of top-level query in original data-query
        </td>
    </tr>
    <tr>
        <td rowspan="2">
