...
:::tip
Jimmer's feature: The developer only needs to write the `data-query`, and the framework automatically generates the `count-query`.  
:::

In addition, the `count-query` can actually be optimized. 

Let's look at a scenario that can be optimized. In the data-query there is a JOIN that satisfies the following conditions:

-   This JOIN operation results in a table that is not used by any `where` conditions, but is used elsewhere in ways that do not affect the number of rows, such as `orderBy`.

-   The direction of this JOIN is from the child table to the parent table *(i.e. one-to-one and many-to-one associations)*, so the join result does not produce duplicate data. That is, this JOIN does not cause the data to grow.

-   This JOIN has a true foreign key *(i.e. there is a foreign key constraint, see [Real and Fake Foreign Keys](../../mapping/base/foreignkey))*, and it is non-null. So this JOIN does not cause the data to shrink.

Then it can be safely removed from the count-query to achieve optimization. 

:::tip
Not only can Jimmer automatically generate `count-query`, it can also optimize `count-query`.
:::
...
-   Spring Boot configuration method:  
    ...
    Add a configuration in `application.properties` or `application.yml` called `jimmer.dialect` with value as the class name of the dialect class provided by Jimmer, such as `org.babyfish.jimmer.sql.dialect.H2Dialect`.
...
-   Query the one-to-many association `Book.store` for the 5 paged objects
