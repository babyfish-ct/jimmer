...
2.  In the final generated SQL, a half join uses only one JOIN statement to join the intermediate table, without further joining the target table with a second JOIN statement.
:::
