...
In the [introduction to dynamic joins](./), we discussed the problems with current technical solutions and briefly introduced the programming style for dynamic JOIN in Jimmer.
...
-   Solve the problem in scenario 1:
    ...
    -   Specify all parameters, `name`, `storeName` and `storeWebsite` are all non-null.
        ...
        :::tip
        This conflict does not cause any problems. Conflicting join objects are merged into one join object.
