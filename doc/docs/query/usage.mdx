...
-   ❸ Call `execute` to execute the query and return data.  
    ...
    No execution happens before this step no matter how the DSL is adjusted to shape the SQL.
    ...
    -   Parameterless version *(as shown in the code)* utilizes Jimmer's connection management *(usually integrating with Spring transaction management)* to get JDBC connection and execute.
