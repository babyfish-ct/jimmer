...
The client can specify dynamic ordering by passing string parameters, which can be converted to `List<Order>` needed by Jimmer using the static method `Order.makeOrders`.

`Order.makeOrders` is defined as:
...
`Order.makeOrders` is very flexible to use, for example:
...
-   Even supports sorting by associations *(one-to-one, many-to-one)*:
