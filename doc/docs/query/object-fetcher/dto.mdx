...
:::caution
Note that if the server queries data in a certain shape not for its own use, but directly as the HTTP response, it is more recommended to directly return the dynamic entity object and use the solutions in [Spring section/Zero Frontend Coupling](../../spring/client) to automatically generate client code with a great developer experience.
:::
