...
`allTableFields` fetches all properties defined in the table, including all scalar properties (same as `allScalarFields`), one-to-one/one-to-many associations based on foreign keys. It does not include one-to-many, many-to-many, one-to-one/one-to-many associations based on middle tables, calculated properties or view properties. 
