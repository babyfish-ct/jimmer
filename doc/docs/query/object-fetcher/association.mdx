...
Since the one-to-many association `Book.store` is based on a real foreign key, the foreign key `STORE_ID` of the current table `BOOK` is the id of the parent object. 
...
-   Jimmer queries the associated objects via separate SQL instead of using LEFT JOIN in the main query SQL.
    ...
    This avoids duplicate results when joining collection associations.
...
`allTableFields` fetches all attributes defined in the table, including all scalar attributes (same as `allScalarFields`), one-to-one/one-to-many associations based on foreign keys. It does not include one-to-many, many-to-many, one-to-one/one-to-many associations based on middle tables, computed properties or view properties. 

:::info
`allTableFields` includes all one-to-one/one-to-many associations based on foreign keys, in addition to `allScalarFields`. But the fetched associated objects only have the `id` property.
:::
...
The equivalence holds when associated objects are not affected by <ViewMore buttonText="filters" variant="text"><FilterRef/></ViewMore>.
:::
