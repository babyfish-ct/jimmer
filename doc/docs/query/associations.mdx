...
But this is not the case. Due to optimizations like [ghost joins](./dynamic-join/optimization#ghost-joins) and [half joins](./dynamic-join/optimization#half-joins), whether association table queries are used or not, similar functionality and performance can be achieved. Using association table queries is completely up to user preference.
...
-   ❷ `association.source` is a [ghost join](./dynamic-join/optimization#ghost-joins) that does not generate a SQL join.
