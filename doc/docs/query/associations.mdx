---
sidebar_position: 12
title: Querying Association Tables Directly
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

## Association Tables Hidden by Object Models  
...
:::tip
The Java code demo here uses a complex `AssociationTable` type to be compatible with Java 8. It is recommended to upgrade Java and use the `var` keyword.
:::
...
-   `target`: The object (Author in this example) corresponding to the foreign key pointing to the inverse side.

:::note

1.  In this example, object fetchers are not used to define the format of Association objects (in fact Association does not support object fetchers), so the `source` and `target` association properties only contain object ids.
...
:::caution
The association object `Association<S, T>` is simple and special, does not support or need [object fetchers](./object-fetcher).  

Note this only applies to `Association<S, T>` itself, its association properties `source` and `target` still support [object fetchers], such as:
...
### 2. Achieve the same functionality without association table subquery  
...
❶  `author.books` is a [semi join](./dynamic-join/optimization#semi-joins) that only generates SQL JOIN from `AUTHOR` to the `BOOK_AUTHOR_MAPPING` table, without further join to the `BOOK` table.
