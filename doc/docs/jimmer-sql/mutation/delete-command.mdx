...
Next, jimmer-sql will look at the annotation [@OnDissociate](../mapping#orgbabyfishjimmersqldissociate) on the many-to-one association property `Book.store`.

1. If the foreign key of `Book.store` is configured as `SET_NULL` by the annotated [@OnDissociate](../mapping#orgbabyfishjimmersqldissociate), then execute the following SQL
...
2. Otherwise, first, execute
    ...
    - If the foreign key of `Book.store` is configured as `DELETE` by the annotated [@OnDissociate](../mapping#orgbabyfishjimmersqldissociate), Use the new delete command to delete these discarded objects, which is actually the automatic recursive execution capability of the delete command.

    - Otherwise, throw an exception.

All of the situations discussed above require the developer to use the annotation [@OnDissociate](../mapping#orgbabyfishjimmersqldissociate) on the `Book.store` property.

However, you can also dynamically specify the `dissociateAction` configuration for the delete command insteading of using the [@OnDissociate](../mapping#orgbabyfishjimmersqldissociate) annotation.
...
Here, calling `setDissociateAction` of delete command has the same effect as statically using the annotation [@OnDissociate](../mapping#orgbabyfishjimmersqldissociate).
...
2. If the delete rule is not only dynamically configured for the save command, but also statically configured in the entity interface through the annotation [@OnDissociate](../mapping#orgbabyfishjimmersqldissociate), the dynamic configuration takes precedence.
:::
