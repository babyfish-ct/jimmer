...
JSqlClient is the entry point for all APIs of jimmer-sql.

## Create JSqlClient

JSqlClient is an immutable object, created based on the Builder pattern.
...
JSqlClient requires an instance and should be shared globally.
...
Then, the JSqlClient object can be injected anywhere to use all the features of jimmer-sql.
...
:::caution
To use the first way, no special configuration of JSqlClient is required.

To use the second way, the ConnectionManager must be configured for JSqlClient. Otherwise an exception will be thrown.

There is no doubt that the second method is more in line with the requirements of business system development and is more recommended. Therefore, it is strongly recommended to configure ConnectionManager for JSqlClient.
:::
...
In this case, why provide the function of dynamically specifying the id generator in JSqlClient?

Dynamically specifying IdGenerator in JSqlClient can override the static configuration of the annotation `@javax.persistance.GeneratedValue` in the entity file, which is very useful for unit testing.
:::
