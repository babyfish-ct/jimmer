...
`SqlClient/KSqlClient` is the entry point of all APIs of jimmer-sql, therefore, SqlClient needs to be created first.

:::danger

The entry points of Java API and Kotlin API are different

- The entry to the Java API is `org.babyfish.jimmer.sql.SqlClient`
