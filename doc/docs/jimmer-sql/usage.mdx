...
:::info
The annotation `@Key` will be explained in other articles, ignore it please.
:::
...
`JSqlClient/KSqlClient` is the entry point of all APIs of jimmer-sql, therefore, JSqlClient needs to be created first.
...
For the topic of cooperation with Spring transaction mechanism, please refer to [JSqlClient](./sql-client).
:::
