...
This cache maps ids to entity objects, which are simple objects and do not contain deep association structures.
...
### 2. Reference association cache

This cache is used for one-to-one and many-to-one associations, mapping the current object id to the id of the associated object
...
:::note
Note: If the current association is a many-to-one association based on a foreign key, and the current object has explicitly included the foreign key property, the cache will not be used.
:::

### 3. List association cache

This cache is used for one-to-many and many-to-many associations, mapping the current object id to a list of associated object ids
...
1. Select a database that supports binlog, enable binlog.
2. Install kafka and create a topic.
3. Use some tools like maxwell, canal or DataBus to send binlog changes to the kafka topic created in the previous step

You can click [here](https://github.com/babyfish-ct/jimmer/blob/main/example/env-with-cache/install.sh) to see the docker install script of the examples.

### Ensure data consistency

To ensure data consistency is very simple, just listen to kafka events and call the relevant API.
...
So far, all the functions we have introduced are query-related, and modification-related functions have not been discussed yet. You can verify by directly executing SQL that modifies the data with any database tool.

In the above code, the second-level cache adapter `RedisBinder` has log printing behavior internally, which can be used to quickly verify data consistency.
