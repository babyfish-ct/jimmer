...
To guarantee data consistency, [Trigger](../mutation/trigger) must be used. 

For cache consistency maintenance, it is recommended to enable [BinLog trigger](../mutation/trigger#enable-binlog-trigger)

:::tip
Once you make sure that [trigger](../mutation/trigger) is enabled, data consistency can be guaranteed automatically.
:::
...
To simplify the discussion, assume that the enabled trigger is [BinLog trigger](../mutation/trigger#enable-binlog-trigger). Subsequent discussions can bypass the application to directly modify the database using any SQL tool.
