...
:::info
As you can see, when saving a <ViewMore buttonText="long association"><LongAssociation/></ViewMore>, even if an illegal id is specified for the associated object *(1000 in this example)*, Jimmer will still create a new associated object using that id.

Therefore, `association id checking` is not an issue at all for <ViewMore buttonText="long associations"><LongAssociation/></ViewMore>.

However, for <ViewMore buttonText="short associations"><ShortAssociation/></ViewMore>, Jimmer is only responsible for creating the association between the current object and the associated object, and will not automatically create non-existing associated objects.
