...
-   All saved objects specify only the id property

-   The save mode of the aggregated root object is explicitly specified as UPDATE_ONLY *(using the update method)*
:::

## Dissociating Join Table Data

There is a many-to-many association between the `BOOK` table and the `AUTHOR` table, with join table `BOOK_AUTHOR_MAPPING`.
...
-   `BookStore-2` needs to dissociate `Book-11` and `Book-12`.
    ...
    But the specific SQL operations are unknown, determined by the dissociation mode configuration of `Book.store` property.
...
-   All associated objects are short associated objects with only id. In fact, you can arbitrarily change the format of associated objects, for example:
    ...
    -   Do not specify `id` property, but specify `key` property *(<ViewMore buttonText="Short Association" variant="text"><ShortAssociation/></ViewMore> expressed in another way)*

    -   Specify `key` property and some properties that are neither `id` nor `key` *(<ViewMore buttonText="Long Association" variant="text"><LongAssociation/></ViewMore>)* 
