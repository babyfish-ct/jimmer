...
-   All saved objects specify only the id attribute

-   The save mode of the aggregated root object is explicitly specified as UPDATE_ONLY *(using the update method)*
:::

## Dissociating Junction Table Data

There is a many-to-many association between the `BOOK` table and the `AUTHOR` table, with junction table `BOOK_AUTHOR_MAPPING`.
...
The `BOOK` table has a foreign key field `STORE_ID` pointing to the `BOOK_STORE` table, forming a many-to-one relationship. This many-to-one association is mapped to the `Book.store` property, and the reverse one-to-many association is mapped to the `BookStore.books` property.
...
-   Dynamic configuration in code, but dynamic configuration only affects the current save command. Dynamic configuration can override static configuration.
...
-   `BookStore-2` needs to dissociate `Book-11` and `Book-12`.
    ...
    But the specific SQL operations are unknown, determined by the dissociation mode configuration of `Book.store` attribute.
...
-   `DELETE`:
    ...
    :::tip
    In fact, step 3 and step 4 are one entity, due to [Delete Command](../delete-command) to be discussed later. 
    :::
...
-   All associated objects are short associated objects with only id. In fact, you can arbitrarily change the format of associated objects, for example:
    ...
    -   Do not specify `id` attribute, but specify `key` attribute *(<ViewMore buttonText="Short Association" variant="text"><ShortAssociation/></ViewMore> expressed in another way)*

    -   Specify `key` attribute and some properties that are neither `id` nor `key` *(<ViewMore buttonText="Long Association" variant="text"><LongAssociation/></ViewMore>)* 

    :::note
    The concepts of short association and long association have been discussed before. If you forget, please review [Save Association](./association#association-types).
    :::
