...
-   ❶ If the id is designated some auto-generation strategy, it is not required. This is also a feature of save commands, see details in [Save Modes/Summary](../save-mode#summary).
    ...
    This seems contradictory, but can be handled simply: If Input DTO's `id` is `null`, the converted entity object will not have an `id` *(although `id` cannot be assigned `null`, dynamic objects always ingore unassinged properties)*.
    :::
