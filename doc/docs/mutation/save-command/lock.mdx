...
-   When modifying objects *(whether explicitly doing UPDATE, or UPSERT determined to be UPDATE)*, Jimmer will compare the `version` passed by the user and the existing `version` in the database for each object. If they differ, an exception will be thrown.
    ...
    In real projects, the version number of optimistic lock is often a hidden field in the form. If the form does not automatically redirect after saving, but keeps the UI unchanged to support multiple submissions, the hidden field should be updated with such return info after each successful save.
    :::
