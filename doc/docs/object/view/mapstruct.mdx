...
Three attributes of this POJO need to be explained:
...
The other attributes are exactly the same as the original entity definition and need no explanation.
...
`BookInput.id`, `BookInput.id`, `BookInput.name` and `BookInput.price` are all non-associated attributes that mapstruct can handle well.
