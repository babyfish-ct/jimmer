Here is the translation of the file into English:
...
Better solutions will be provided later.
:::
...
1.  Create the directory `com/yourcompany/yourproject` in `src/main/dto`, i.e. create a directory consistent with the package name
...
Shapes can be modified by the `input` modifier, for example:
...
-   In fact, there is another kind of DTO object without direct Input/Output semantics, which is often used as the query specification parameter for complex dynamic query APIs. 
    ...
    :::note
    Jimmer makes no recommendation on whether this type of query rule parameter should be defined as input type. But it is recommended that this type ends with `Specification`/`Spec`.
    :::  

:::info
Compared with Output DTO:

-   The id attribute in the input DTO is nullable if the id attribute of the entity is configured with auto growth strategy.

-   The input DTO can only define attributes that can be saved, such as simple attributes, ordinary ORM association attributes and view attributes.
    Attributes that cannot be saved, such as transient attributes, formula attributes, calculated attributes, and remote associations, cannot be defined, otherwise compilation errors will occur. 
:::
...
`input-only`/`inputOnly` works similarly to `input` and is used to modify input types, but only provides the ability to convert from DTO objects to entity objects, not the ability to convert from entity objects to DTO objects.
...
This means that the DTO only maps the three attributes `id`, `name` and `editon` of the entity, as follows:
...
The DTO language supports a special macro attribute, `#allScalars`, which means mapping all scalar attributes in the entity.
...
Same as the official example `Book` type definition:

-   `Book` itself has scalar attributes `id`, `name`, `edition` and `price`

-   `Book` inherits `TenantAware`, `TenantAware` interface defines attribute `tenant`  

-   `Book` also inherits `CommonEntity` *(multiple inheritance)*, `CommonEntity` interface defines attributes `createdTime` and `modifiedTime`

`#allScalars` means automatically mapping all scalar attributes of the entity, including inherited ones.  

If this is not the behavior you want, then you can use `allScalars(Book)` to mean mapping only all scalar attributes defined by the `Book` type itself, not including inherited attributes.
...
:::info
-   If using the `#allScalars` macro, it must be defined as the first attribute, otherwise a compilation error will occur.
...
The `#allScalars` macro and inheritance discussed later will batch map multiple attributes. Negative attributes can remove some attributes:
...
- `#allScalars` will add attributes `createdTime`, `modifiedTime`, `tenant`, `id`, `name`, `edition` and `price`.

- `-tenant` removes the `tenant` attribute from it, finally adding attributes `createdTime`, `modifiedTime`, `id`, `name`, `edition` and `price`.

:::info
The negative attribute specifies the actual name of the DTO attribute, not the entity attribute name. Here the two are the same, so there is no difference.
...
By default, the nullability of DTO attributes is the same as that of the original attributes. But we can use `?` or `!` to modify the nullability of DTO attributes.

### 7.1 `?` 

We can use `?` to make the DTO attribute nullable.
...
```
In the entity `Book.price` cannot be null, while the corresponding DTO attribute `UpdateBookInput.price` can be null. This seems contradictory, but Jimmer can handle such contradictions *(without discussing whether it is appropriate to use this feature in specific scenarios)*.

- When converting an entity to a DTO, if `Book.price` is not loaded *(although the attribute cannot be null, dynamic objects can have unloaded attributes)*, then `UpdateBookInput.price` in the converted DTO is null.

- When converting a DTO to an entity, if `UpdateBookInput.price` is null, then `Book.price` in the converted entity is in an unloaded state, that is, this attribute of the dynamic object is not assigned at all.

You can even use `?` for `allScalars` to make all automatically mapped non-associated attributes nullable:
...
For entity attributes that can be null in the first place, there are three states: non-null, null and unloaded *(unknown)*; while static DTO attributes only have two states: non-null and null. That is, introducing static DTO types itself has already led to loss of information.

Therefore, for entity attributes that can be null in the first place, no matter how the corresponding DTO attribute is set, after DTO conversion, the nullable attribute of the dynamic entity object must be assigned *(may be null)*, but the unloaded state cannot appear. If passed to [save command](../../mutation/save-command), null will definitely cause the corresponding field in the table to be updated, rather than not updated like non-null attributes that have not been assigned.

In fact, in addition to DTO types with direct Input/Output semantics, complex dynamic queries often involve a kind of query specification parameter, such as [AuthorSpecification](https://github.com/babyfish-ct/jimmer/blob/main/example/java/jimmer-sql/src/main/dto/org/babyfish/jimmer/sql/example/model/Author.dto) in the example.

For query specification parameters `Specification`, this feature is very suitable, because each query parameter of dynamic query should be optional, unless users have to be required to provide some query conditions due to performance issues.
...
We can use `!` to modify the DTO attribute to be non-null. But its usage is limited and can only be used in the following two cases:

- Modify the id attribute in `input`/`input-only` types

    As mentioned earlier, if the id attribute is configured with auto growth strategy, the corresponding input DTO type will set the id attribute to nullable.

    However, this behavior does not always match the user's expectations. `!` can be used in `input`/`input-only` types to modify the id attribute to make it non-null, for example:
...
- In `input-only`, convert a nullable entity attribute to non-null, for example:
    ...
    ```
    :::info
    The attribute in the entity can be null, but the corresponding attribute in the DTO is non-null, so it is impossible to support converting entity objects to DTO objects, only converting DTO objects to entity objects.
...
The `as` keyword can be used to set an alias for the attribute, so that the entity attribute name and DTO attribute name can be different.

`as` has two usage, it can set alias for one attribute precisely, it can also set aliases for multiple attributes vaguely.

### 8.1. Rename Single Attribute
...
### 8.2. Rename Multiple Attributes
...
Most attributes can be defined in the `as() {...}` block, including `#allScalars`, for example: 
...
:::caution
`^` and `$` cannot appear at the same time, because it means renaming a single attribute precisely, which duplicates existing functionality
:::

## 9. Association Attributes  

Handling association attributes is a very powerful feature of the DTO language. There are three usages:
...
- Renaming
    ...
    If you want to rename association attributes, the `as` clause should be before association definition block, for example:
...
### 9.2 Recursive Association

For recursively associated attributes like `TreeNode.parent` or `TreeNode.childNodes` in the attached example, the asterisk `*` can be used to mark the corresponding associated attribute of the DTO as a recursive attribute.
...
For <ViewMore buttonText="short associations" variant="text"><ShortAssociation/></ViewMore>, of course we choose to define [@IdView](../../mapping/advanced/view/id-view) properties in entities, for example:
...
Therefore, the DTO language supports calling the `id` function on associated attributes to achieve the same purpose. 
:::
...
The `flat` function can only be used for associations *(one-to-one or many-to-one)*, not collection associations *(one-to-many or many-to-many)*. It is used to lift the attributes of the associated DTO to the current DTO type to obtain a flat object without associations.
...
As mentioned earlier, this example is wrong, because the aggregate root and associated object both have the attributes `id` and `name`. Unconditionally extracting the attributes of the associated object to the aggregate root must inevitably lead to name conflicts, eventually causing the DTO language to report compilation errors.

The correct approach is to combine it with the `as(...) {}` block that renames multiple user attributes vaguely. Let's look at a new example:
...
This way, we get a flat DTO type with 4 attributes: `id`, `name`, `parentId` and `parentName`. 
...
This way, we get a flat DTO type with 6 attributes: `id`, `name`, `parentId`, `parentName`, `grandParentId`, `grandParentName`.
...
Please refer to <ViewMore buttonText="Controversy over whether output DTOs should use flat mode" variant="text"><FlatProblem/></ViewMore>
...
- Due to the existence of negative attribute features, derived DTO type definitions may have fewer attributes than base DTO type definitions
...
### 11.1 Getting Started with Custom Fields

In the examples we have discussed so far, all DTO attributes are mapped from entity attributes and become mapping attributes.

In addition to mapping attributes, DTO types also support custom attributes:
...
- ❶ Import types to simplify subsequent custom attribute definitions

- ❷ ❸ Define custom attribute definitions

    Custom attributes use syntax similar to Kotlin, `?` can be used to explicitly make the attribute nullable, otherwise it cannot be null.
    ...
    :::caution
    The custom attributes here allow null. To define non-null attributes *(i.e. remove `?`)*, the `input-only`/`inputOnly` modifier must be used
    :::
...
For other non-associated fields, the query condition is a single value query condition like `eq` and `ilike`, so simply using `#allScalars` mapping is sufficient.

However, the query conditions corresponding to `createdTime` are relatively complex, which is a range query, and cannot simply map the `createdTime` attribute in the mapped entity, but needs two boundary attributes: `minCreatedTime` and `maxCreatedTimeExclusive`.

It can be seen that for query specification parameters like `AuthorSpecification`, custom attributes can bring great convenience, mapping attributes that can be mapped, and customizing attributes that cannot be mapped.

Therefore, the following is recommended:

- Specificiation DTO: Encouraged to use, custom attributes are designed for this type.
...
- Output DTO: Not recommended
    ...
    - Second, even if Output DTOs are used, Output DTO objects will only have the attributes of entity objects, unless you are willing to manually populate custom attributes.

    :::tip
    For output types, custom attributes are entity-level things *(so Output DTOs are not required)*.
    
    Please refer to *[View Attributes](../../mapping/advanced/view)、[Simple Calculated Properties](../../mapping/advanced/calculated/formula)、or [Complex Calculated Properties](../../mapping/advanced/calculated/transient)*
    :::
...
In addition to built-in types, other types must be written in full when referenced, or imported at the beginning of the file using import statements, otherwise they will be considered to be in the same package as the current entity.
...
You can use annotations to modify DTO types, mapping attributes and custom attributes in DTO language:
...
BookView {
    ...
    # highlight-next-line
    @A authors @B {
        ...omit association object attributes... 
    }
...
- `@A` modifies the `authors` attribute  

- `@B` modifies the anonymous association type
:::

### 12.2 Annotation Override

- As mentioned earlier, DTO language supports annotations to directly control the annotations of the generated DTO types

- If a DTO attribute is not modified by any annotation, the annotations on the corresponding attribute of the original entity will be copied *(if the DTO attribute is mapped, not custom)*.
...
Unlike Java, no matter how many parameters the annotation has, the `value` parameter can be abbreviated, as long as the abbreviated attribute is configured first.
