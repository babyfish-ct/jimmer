...
-   The id property in the input DTO is nullable if the id property of the entity is configured with auto growth strategy.

-   The input DTO can only define properties that can be saved, such as simple properties, ordinary ORM association properties and view properties.
...
-   `Book` inherits `TenantAware`, `TenantAware` interface defines property `tenant`  
...
- Modify the id property in `input`/`input-only` types
    ...
    As mentioned earlier, if the id property is configured with auto growth strategy, the corresponding input DTO type will set the id property to nullable.
...
- In `input-only`, convert a nullable entity property to non-null, for example:
...
You can use annotations to modify DTO types, mapping properties and custom properties in DTO language:
...
- `@A` modifies the `authors` property  
...
- If a DTO property is not modified by any annotation, the annotations on the corresponding property of the original entity will be copied *(if the DTO property is mapped, not custom)*.
