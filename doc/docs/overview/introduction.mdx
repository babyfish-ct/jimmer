...
1.  DTO explosion problem
    ...
    To do this, Jimmer's solution is to query arbitrarily shaped data structures <ObjectFetcher/>
...
2.  Complex form problem
    ...
    To do this, Jimmer's solution is to save arbitrarily shaped data structures <SaveCommand/>
...
The DTO language is a simple language that comes with Jimmer.

Even if in some special scenarios, we cannot directly use unified ORM entities and have to use DTO objects, the development cost is only to use the DTO language to define the shape of these DTOs.
All the rest functions such as

-   Java/Kotlin definitions for DTOs

-   Data transformation between entities and DTOs

-   The DTO query logic

are fully automated, and the pain caused by DTO explosions in traditional technical solutions is completely unfelt.

<Button href="../object/view/dto-language" size="small" variant="outlined">前往看看</Button>

### 8. Remote association
...
[Query arbitrary shaped data structures](../query/object-fetcher)
...
[Entity object calculated properties](../mapping/advanced/calculated/transient)
...
[Entity object remote associations](../spring/spring-cloud)
...
[Intra-transaction trigger](../mutation/trigger#触发器分类)
...
[Immutable objects](../object/immutable)
