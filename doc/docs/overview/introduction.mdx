...
1.  DTO explosion problem
    ...
    Even for the same data type *(aggregate root)*, different business functions expect different data formats to be queried, which often leads to project needing to define a large number of DTO types<ViewMore buttonText="DTO explosion" variant="text"><img src={DtoExplosion}/></ViewMore>。
...
3.  cache
    ...
    - The cache should not only have the simplest object cache, but also <ViewMore buttonText="Association cache" variant="text"><img src={AssociationCache}/></ViewMore> and <ViewMore buttonText="Computational cache" variant="text"><img src={CalculatedCache}/></ViewMore>

    - The global filtering capability represented by the permission system often allows different users to see different data, so it should also be <ViewMore buttonText="Let different users see different caches" variant="text"><img src={MultiViewCache}/></ViewMore>
...
- <ViewMore buttonText="Object Cache" variant="text"><img src={ObjectCache}/></ViewMore>、<ViewMore buttonText="Association Cache" variant="text"><img src={AssociationCache}/></ViewMore> and <ViewMore buttonText="Computational Cache" variant="text"><img src={CalculatedCache}/></ViewMore>

- <ViewMore buttonText="MultiView Cache" variant="text"><img src={MultiViewCache}/></ViewMore>, different users see different caches, often in combination with permission systems
...
<Button href="../spring/spring-cloud" size="small" variant="outlined"> go and take a look</Button>
