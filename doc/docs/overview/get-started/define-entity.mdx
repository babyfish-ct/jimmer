...
-   Author
    ...
    :::info
    See **TODO** for details on mapping custom scalars.
    :::
...
-   Entities have non-null ID like `long`, not nullable ID like `Long`, otherwise framework will error.
...
-   Mirroring is optional, bidirectional is not required.
...
-   For owner, `@JoinTable` can specify join table:
    ...
    :::note
    Here `@JoinTable` uses all default values, so it can be omitted for readability.
    :::
...
Mirroring many-to-many is optional, bidirection is optional. But if bidirectional, one side must be owner and one mirror.
...
1.  Dynamicity
    ...
    See **TODO** for details on dynamicity.

    - Benefits of dynamicity

        - Easily express arbitrary complex data structures. Entities can be partial, complete, or aggregate root of complex tree.

        - Since data structure is flexible, ORM can load, save, query entire object graph in one go instead of individual entities. 

        - For querying, dynamic entity graphs can directly return from HTTP services without needing DTOs.

            :::tip
            Server utilizes dynamicity to simplify development.

            But Jimmer can restore static typing on client by auto-generating TypeScript DTOs for all use cases.
...
2.  Immutability
    ...
    Mutable shared data can lead to inadvertent changes and inconsistencies. Immutable data avoids this risk.

    Enjoying immutable objects while suppressing downsides is hard, especially for complex object graphs like in ORM. 

    Here is an English translation of the Chinese text:
    ...
    See **TODO** for details on how to create and "modify" immutable objects.
