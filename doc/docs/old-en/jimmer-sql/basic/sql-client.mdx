...
:::caution
If you want to use jimmer's spring-boot-starter and spring-data-jimmer to simplify usage without creating SqlClient yourself, you don't need to read this document.
:::
...
Multiple `ScalarProvider`s can be added to JSqlClient, each `ScalarProvider` tells jimmer how to handle a custom data type.
