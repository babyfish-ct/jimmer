---
sidebar_position: 2 
title: Generate Frontend Exceptions
---
...
- Share one business exception class across the entire application, with built-in error codes that the client can immediately understand
...
So, Jimmer chooses a compromise: dividing business exceptions into multiple clusters, with each cluster using one set of Error Codes.

Jimmer uses enums to define exception clusters, where the enum also serves as the error code for that cluster, e.g.
...
The `@org.babyfish.jimmer.error.ErrorFamily` annotation indicates that this enum represents a cluster of business exceptions, and the enum itself serves as the error code for this cluster. 
...
:::info
For enums used to declare exception clusters, you can choose to use `ErrorCode` or `Error` as a special suffix. 
...
- ❷ The error code type for this cluster of exceptions is `UserInfoErrorCode` 
...
:::info
The `@org.babyfish.jimmer.client.ThrowsAll` annotation allows the REST API to throw all exceptions in the cluster.
:::

### Allow Some Exceptions in Cluster

Let's locate another business exception cluster:
...
To be able to declare that a REST API can only throw some of the exceptions in this cluster, define the following annotation:
...
```json
{
    ...
    // highlight-next-line
    "debugInfo":{
        "message":"Illegal geography position which is already marked",
        "stackFrames":[
            "org.babyfish.jimmer.spring.java.bll.GeographyException.illegalPosition(GeographyException.java:35)",
        ...
        "causeBy":{
            "message":null,
            "stackFrames":[
                "org.babyfish.jimmer.spring.java.bll.ErrorService.validatePosition(ErrorService.java:28)",
...
- ❷ The enum name decorated with `@ErrorFamily` is the `family` field in client code, used to identify which cluster the error belongs to
