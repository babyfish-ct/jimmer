...
| jimmer.pretty-sql | boolean | false | Whether to format SQL, affects both execution and logs |
...
| jimmer.transaction-cache-operator-fixed-delay | int | 5000 | Retry interval for failed or excessive cache cleanup tasks when trigger type is `TRANSACTION_ONLY` (not relying on external CDC) |
...
| jimmer.error-translator.http-status | HttpStatus | INTERNAL_SERVER_ERROR | Unified HTTP status of server exceptions on client side |
...
| jimmer.client.java-feign.indent | int | 4 | Indentation of automatically generated TypeScript code |
