...
:::caution
Note that if the data of a certain shape queried by the server is not for its own use, but to be directly returned as the HTTP request return value, it is more recommended to directly return the dynamic entity object and use the scheme in [Spring Section/Frontend-Backend Decoupling](../../spring/client) to automatically generate client code with high development experience.
:::

## Define DTO shape

This article focuses on how to query static DTO types, not a systematic introduction to the DTO language. Please refer to [Object Section/DTO Mapping/DTO Language](../../object/view/dto-language) for a complete DTO language.
...
-   Recommended approach
    ...
    1.  First, define a method-level generic parameter `V`, which generic parameter must extend `org.babyfish.jimmer.View<Book>` *(this is very important, otherwise Jimmer will remind developers to do so via exceptions)*.
