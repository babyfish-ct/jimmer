...
2. Unlike other ORMs, it supports not only object caching, but also associative caching. The two are combined behind the scenes and can query object trees with any depth and breadth, especially when recursively querying self-associative properties.
