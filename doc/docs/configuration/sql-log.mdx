...
To improve the readability of this document, the above log is manually wrapped, which is not the actual situation.
:::
...
-   `purpose`: The reason why the SQL is executed, which is an enum with the following values:
    ...
    -   `LOAD`: Query executed automatically by Jimmer to get the value of associated properties or computed properties in [object fetcher](../query/object-fetcher).
...
In the previous example, the SQL log is printed in one line, *(To improve the readability of this document, the log in the previous example is manually wrapped, which is not the actual situation.)*, which is poorly readable.
