...
In the figure, we list three business perspectives as examples. For any business perspective, it can freely decide whether each business perspective cares about certain entities, associations, or even attributes.

To achieve this kind of data structure shape tailoring that is accurate to attributes, the only way is to make entity objects support dynamism. 

:::info
The dynamism of Jimmer entities is not the kind of dynamism that gives up spell checking and type safety like dynamic languages. It is just that [the `fetch = FetchType.LAZY` of the @Basic annotation in Hibernate](https://docs.jboss.org/hibernate/orm/6.2/userguide/html_single/Hibernate_User_Guide.html#BytecodeEnhancement) is extended to any attribute. 

So Jimmer entities are 100% strongly typed, with spell safety, type safety, and even null safety (limited to Kotlin). It just allows any attribute of an object to be missing.
:::
...
:::tip
Use a unified entity model to express any data format required by the business, rather than define DTOs for each data format required by the business scenario.

This is the key for Jimmer to avoid both the lack of flexibility of traditional ORM and the repetitive labor of MyBatis.
:::
