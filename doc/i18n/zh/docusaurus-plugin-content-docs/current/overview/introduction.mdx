...
1.  DTO爆炸问题
    ...
    即便对于同一种数据类型 *(聚合根)* 而言，不同业务功能期望查询的数据格式也不同，这往往导致项目需要定义大量的DTO类型，形成<ViewMore buttonText="DTO爆炸问题" variant="text"><img src={DtoExplosion}/></ViewMore>。
...
3.  缓存
    ...
    -   缓存不应该仅有最简单的对象缓存，还应有<ViewMore buttonText="关联缓存" variant="text"><img src={AssociationCache}/></ViewMore>和<ViewMore buttonText="计算缓存" variant="text"><img src={CalculatedCache}/></ViewMore>。

    -   以权限系统为代表的全局过滤能力常常让不同用户看到不同的数据，因此，也应该<ViewMore buttonText="让不同用户看到不同缓存" variant="text"><img src={MultiViewCache}/></ViewMore>
...
-   <ViewMore buttonText="对象缓存" variant="text"><img src={ObjectCache}/></ViewMore>、<ViewMore buttonText="关联缓存" variant="text"><img src={AssociationCache}/></ViewMore>和<ViewMore buttonText="计算缓存" variant="text"><img src={CalculatedCache}/></ViewMore>

-   <ViewMore buttonText="多视图缓存" variant="text"><img src={MultiViewCache}/></ViewMore>，不同用户看到不同的缓存，往往和权限系统结合使用
