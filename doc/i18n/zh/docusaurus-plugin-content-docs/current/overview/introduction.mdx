...
4.  动态表连接
    ...
    传统的数据库访问技术只关注动态where条件，不专注动态表join，这导致复杂的动态查询<ViewMore buttonText="难以书写" title="缺失动态表连接的问题" variant="outlined"><DynamicJoinProblemPanel/></ViewMore>
...
<Button href="../object/view/dto-language" size="small" variant="outlined">前往看看</Button>
