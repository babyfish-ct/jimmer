...
:::caution
除非为了兼容遗留系统中已有的DTO类型，否则不推荐使用MapStruct，更推荐使用[DTO语言](./dto-language)，原因如下

-   不可忽略的开发成本

    Jimmer以可以表达任意形状数据结构的动态实体对象为系统核心，让系统同时具备便捷性和灵活性。
    
    因此，Jimmer的DTO只是为兼容JVM现有生态的而创建的极其廉价的装饰物，并非系统核心，即使需要，也追求可以忽略不计的开发成本。

    然而，手动定义DTO并利用MapStruct完成映射，明显具备不可忽略的开发成本。

-   只能作为Input DTO

    DTO语言自动生成的DTO类型具备内置的[对象抓取器](../../query/object-fetcher)，因此可以作为查询的输出类型，请参见：

    -   [对象抓取器/DTO查询](../../query/object-fetcher/dto)

    -   [Spring Data/查询DTO](../../spring/repository/dto)

    然而，手动定义并采用MapStruct映射的DTO没有对应的[对象抓取器](../../query/object-fetcher)定义，只支持和动态实体相互转化。
    所以仅适合作为Input DTO使用，不适合作为Output DTO。

-   和Kotlin项目配合的问题

    -   [mapstruct框架](https://mapstruct.org/)是基于`apt`*(Annotation Processor)* 的。
