...
:::caution
对于Kotlin而言，不推荐在此方案, [DTO语言](./dto-language)更值得推荐。
...
-   DTO语言自动生成的DTO类型具备内置的[对象抓取器](../../query/object-fetcher)，因此

    -   可以被底层API查询，请参见[对象抓取器/DTO查询](../../query/object-fetcher/dto)

    -   可以被Spring Data风格用户代码查询，请参见[Spring Data/查询DTO](../../spring/repository/dto)

    然而，手动定义并采用MapStruct映射的DTO没有这些功能，它只支持和动态实体相互转化。
:::
