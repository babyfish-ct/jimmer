...
我们可以采用`!`修饰DTO属性，让其非null。但其使用场景受限，只能用于一向两种情况。
...
-   ❷ ❸ 定义自定义属性的定义
    ...
    :::cuation
    这里的自定义的属性是可null，如果要去掉`?`以定义非null的自定义属性，必须使用`input-only`/`inputOnly`修饰符
    :::

    -   对于Kotlin项目而言，生成与之完全对应的类型

    -   对于Java项目而言，除了顶层类型的`?`会被用于在目标Java类中生成`@org.jetbrains.annotations.Nullable`注解外，内部范型的`?`会被忽略
...
-   实体属性本身可以拥有注解，其中，非jimmer注解会被复制到被生成的DTO类型中

-   DTO语言也支持注解，直接控制被生成的DTO类型的注解

在最终生成的DTO类型中，这两部分注解会合并。如果二者冲突，由开发人员直接负责
