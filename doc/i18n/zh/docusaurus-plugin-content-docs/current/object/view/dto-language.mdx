...
如果原始对于可递归的属性，比如，附带例子中的`TreeNode.parent`或`TreeNode.childNodes`，可以使用星号`*`将DTO对应的关联属性标记为递归属性。
...
对于<ViewMore buttonText="短关联" variant="text"><ShortAssociation/></ViewMore>而言，我们当然选择为在实体中定义[@IdView](../../mapping/advanced/view/id-view)属性，例如
...
:::tip
然而，我们不能总是寄希望于实体类型总是声明了[@IdView](../../mapping/advanced/view/id-view)属性，更好的方案不应该又此假设。

因此，DTO语言可以对关联属性调用`id`函数来达到同样目的
:::
...
事实上，这个例子是错误的，因为聚合根和关联对象都有属性`id`和`name`，无条件把关联对象的属性往聚合根上提取，必然导致名字冲突，最终导致DTO语言编译报错。
...
甚至还可以嵌套使用`flat`函数，将来多级关联转化为平台对象
...
`flat`的缺点也很明显，DTO语言虽然给予了强大的支持，但并不推荐盲目采用，
请参见<ViewMore buttonText="对是否采用flat模式的建议" variant="text"><FlatProblem/></ViewMore>
...
对于生成的Java/Kotlin类型`DefaultRobotView`类而言，并不会任何Java/Kotlin继承关系，这是因为：
