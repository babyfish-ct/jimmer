...
:::note
**在关联对象未使用[全局过滤器](./filter)的情况下**，`allTableFields()`返回的就是默认对象格式，所以以下两个select调用等价
:::
...
:::caution
如果关联对象使用了[全局过滤器](./filter)，二者不再等价。

- 查询简单的表对象：不会应用[全局过滤器](./filter)，只要外键非null，就会把关联属性设置为只有id的父对象。

- `allScalarFields()`：会应用[全局过滤器](./filter)，即使外键非null，就可能因关联对象不符合过滤规则而将父对象设置为null。
:::
...
2.
    使用了字段级过滤器后，该字段的[外部缓存](./cache)会失效。

    如果不想让关联缓存失效，可以使用[缓存友好的全局过滤器](./filter/#过滤器和缓存)。
