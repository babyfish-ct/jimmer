...
为了把开发人员从这两个繁重的任务中解脱出来，框架的缓存机制被设计得足够强大，对开发人员透明。
...
此缓存用于用户自定义的计算字段，这种缓存会在[计算属性](./calculated-prop)一文中详细讲解，本文不予关注。
...
通过前文的描述，我们可以为任何类型指定对象缓存，也可以为任何属性置顶关联缓存。

然而，实际项目中，实体类型和关联属性可能会非常多，逐个配置会显得非常冗长。隐式配置可以横好地应对这种情况。
