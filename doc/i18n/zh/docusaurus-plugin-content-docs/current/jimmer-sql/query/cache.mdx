...
要保证一致性，必须使用[触发器](../mutation/trigger)。

对于缓存一致性维护，推荐启用[BinLog触发器](../mutation/trigger#启用binlog触发器)

:::tip
一旦确保[触发器](../mutation/trigger)生效，可以自动拥有了缓存一致性。
:::
...
为了简化讨论，假设启用的触发器是[BinLog触发器](../mutation/trigger#启用binlog触发器)。后续的讨论就可以绕过应用使用任何SQL工具修改数据库。
