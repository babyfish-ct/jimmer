...
:::caution
如果你想使用jimmer的spring-boot-starter和spring-data-jimmer简化使用方式，无需自己创建SqlClient，可以不阅读本文档。
:::
...
可以为SqlClient添加多个`ScalarProvider`，每个`ScalarProvider`告诉Jimmer如何处理一种自定义数据类型。
