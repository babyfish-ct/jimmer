...
右上图为用户期望的数据结构。为了标识每个对象的身份以实现，每个对象要么指定[id](./jimmer-sql/mapping#id)，要么指定[业务key](./jimmer-sql/mapping#key)和[id增长策略](./jimmer-sql/mapping#generatedvalue)。
...
- 虽然保持指令是黑盒化的高级API，但是开发人员人员可以精细拦截其中细节动作（图中具有颜色的变化部分），因为Jimmer支持强大的[触发器](./jimmer-sql/mutation/trigger)机制
:::
