...
|associatedIdEq或id|如果关联非集合，`propId`；否则，编译报错|
|associatedIdNe|如果关联非集合，`excludedPropId`；否则，编译报错|
|associatedIdIn|如果关联非集合，`propIds`；否则，编译报错|
|associatedIdNotNull|如果关联非集合，`excludedPropIds`；否则，编译报错|
