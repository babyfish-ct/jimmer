...
:::caution
这里会出现一系列现在没讲解的概念，它们都会在后续文档中讲解。这里先做罗列，请读者先忽略这些名词，等了解了所有概念后再回来查看。
:::
...
-   `@org.babyfish.jimmer.sql.OneToMany`和`org.babyfish.jimmer.sql.@ManyToMany`属性必须非null

-   `@org.babyfish.jimmer.sql.OneToOne`和`org.babyfish.jimmer.sql.@ManyToOne`属性在某些场合下必须可null
