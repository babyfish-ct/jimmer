...
public interface BookStoreRepository implements JRepository<BookStore, Long> {
    ...
    default List<Tuple2<Long, BigDecimal>> findIdAndAvgBookPrice(
        Collection<Long> ids
    ) {
...
1.  限定查询范围，仅对当前需要计算的书店计算它所属书籍的平均价格，而非数据库中所有书店
...
由于之前我们已经讲解了`BookStore.avgPrice`，对计算属性、Resolver和Repository之间的联系有了了解。所以，这个例子不再采用一边开发一边讲解的顺序，而是直接采用实际开发的顺序：
...
public interface BookStoreRepository implements JRepository<BookStore, Long> {
    ...
    default List<Tuple2<Long, Long>> findIdAndNewestBookId(
        Collection<Long> ids
    ) {
...
3.  限定查询范围，仅针对当前需要查询最新版本书籍的，而非数据库中所有书店。
