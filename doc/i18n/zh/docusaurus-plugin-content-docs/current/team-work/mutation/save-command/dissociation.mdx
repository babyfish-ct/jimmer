...
-   对聚合根对象一律明确指定UPDATE_ONLY模式 *(调用update方法)*
:::
...
如果中间表外键为真 *(请参见[真假外键](../../mapping/base/foreignkey))*，Jimmer无需检查用户指定的`authorId`是否合法，生成的SQL是最简单的，只有三条
...
`BOOK`表有一个指向`BOOK_STORE`表的外键字段`STORE_ID`，形成多对一关系。这个多对一关联被映射成了属性`Book.store`，反过来的一对多关联被映射成了一对多关联`BookStore.books`。
...
-   `SET_NULL`
    ...
    2.  将隶属`BookStore-2`且id不是9或10的所有书籍 *(本例为`Book-11`和``Book-11``)* 的外建清空。
...
-   `DELETE`
    ...
    :::tip
    事实上，步骤3和步骤4是一个整体，因后续要讨论的[删除指令](../delete-command)所致。
    :::
...
-   关联对象全部是只有id的对象。事实上，你可以随意改变关联对象的格式，比如

    -   不指定`id`属性，而指定`key`属性 *(<ViewMore buttonText="短关联" variant="text"><ShortAssociation/></ViewMore>的另外一种表达方式)*

    -   指定`key`属性以及一些既非`id`也非`key`的属性 *(<ViewMore buttonText="长关联" variant="text"><LongAssociation/></ViewMore>)*

    :::note
    短关联和长关联这两个概念已经讨论过，如果忘记，请回顾[保存关联](./association#关联类型)
    :::

本文如此简化的例子，只是为了让保存指令生成的SQL尽可能简单，让读者能快速明白脱钩操作的特性。

至于格式更复杂的关联对象，读者可以自行尝试，也可以直接参考官方例子[example/java/save-command](https://github.com/babyfish-ct/jimmer/tree/main/example/java/save-command)或[example/kotlin/save-command-kt](https://github.com/babyfish-ct/jimmer/tree/main/example/kotlin/save-command-kt)。但这些功能仍然存在，无外乎生成SQL语句更多，被隐藏的细节更繁琐而已。
:::