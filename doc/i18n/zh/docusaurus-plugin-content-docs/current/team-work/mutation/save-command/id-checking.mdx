...
对于<ViewMore buttonText="长关联" variant="text"><LongAssociation/></ViewMore>而言，Jimmer会先创建不存在的关联对象，然后建立当前对象和新关联对象之间的关联。例如：
...
:::info
可见，在保存<ViewMore buttonText="长关联" variant="text"><LongAssociation/></ViewMore>时，即使为关联对象设置了数据库中不存在的id *(本例中为200)*，Jimmer也会按照此id创建新的关联对象，长关联根本不存在`关联id检查问题`。

但是，对于<ViewMore buttonText="短关联" variant="text"><ShortAssociation/></ViewMore>而言，Jimmer只负责在当前对象和关联对象之间创建关联，并不会自动创建不存在的关联对象。
...
用户可以配置是否检查短关联对象的id，下面我们讨论各种属性在检查和不检查情况下的行为
...
-   没有目标外键的属性
    ...
    -   不检查
        ...
        假如1000，10001是数据库中不存在的id，那么这条update语句只会影响两条存在的数据，不存在的两条数据会被自动忽略。
...
-   有目标外键的属性
    ...
    -   验证，由Jimmer报错。好处是异常信息和异常类型明确，坏处是多执行一条SQL查询，坏处
    :::

:::tip
因此，只要项目不是对性能优化到极致的那种，建议此检查机制对所有属性全开。
:::
