...
对于<ViewMore buttonText="长关联"><LongAssociation/></ViewMore>而言，Jimmer会先创建不存在的关联对象，然后建立当前对象和新关联对象之间的关联。例如：
...
这个例子中`Author`类型的key为`firstName`和`lastName`。在实际项目中，这样的key不太合理，但作为一个例子无关紧要，别太较真。
...
from AUTHOR tb_1_
where
        tb_1_.FIRST_NAME = ? /* Svetlana */
    and
        tb_1_.LAST_NAME = ? /* Isakova */
...
:::info
因此，<ViewMore buttonText="长关联"><LongAssociation/></ViewMore>没有非法关联id的问题，
:::
