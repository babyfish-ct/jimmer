...
你也可以让它们用缓存模式启动，只需要将spring-boot的profile设置为`maxwell`或`debezium`即可。然而，缓存模式需要安装外部环境。
...
2.  安装外部环境
    ...
    为了让例子程序展现出Jimmer强大的缓存一致性维护功能，需要利用业界已有的技术把数据库的变化推送给Jimmer。
    
    :::tip
    事实上，通过外部手段把数据库的变化推送给Jimmer并不是必须的，也可以让Jimmer通过其的内部机制发现数据库的变化。
    
    -   Jimmer自己捕获数据变化的功能叫`Transaction触发器`
    
    -   通过外部手段把数据库的变化推送给Jimmer的功能叫`BinLog触发器`
    
    请参见[触发器分类](../mutation/trigger#触发器分类)。

    `BinLog触发器`既具有更好的性能，又具备更好的普适性 *(即使绕过Jimmer通过其他方式修改数据库，也可以被拦截)*，是推荐方案。
    
    因此，官方附带例子使用`BinLog触发器`来同步缓存，我们需要通过外部手段把数据库的变化推送给Jimmer。
    :::

    附带例子支持两种外部环境，你可以随意选择一种方式体验例子的缓存模式
