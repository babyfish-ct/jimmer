...
-   Jimmer创建动态对象返回给开发人员，即，查询任意形状的数据结构

-   开发人员创建动态对象传递给Jimmer，即，保存任意形状的数据结构
...
<ViewMore title="Java/Kotlin面对复杂不可变对象的问题" buttonText="Java/Kotlin面对复杂不可变对象的问题">
...
-   对于Set、Map这里以来hashCode的集合容器而言，迫切需要对象的不可变性。

    一旦集合容器保存了可变对象，开发人员就必须小心翼翼地确保不去修改被这些容器共享的数据。如果不慎犯了错误，通常需要debug跟踪来发现问题，这往往会浪费时间影响效率。

-   其实，除了集合容器外，实际开发中还有其他需长期持有某些对象的场合场合，比如：

    -   利用Web Session中长期持有某些数据

    -   利用一级缓存，即，使用JVM内存的进程内缓存，长期持有某些数据

    和集合容器能作为某个模块的私有数据不同，WebSession和Cache这类结构是对外暴露的，具有很强的`public`特性，会更麻烦。
