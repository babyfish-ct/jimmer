...
-   使用Jimmer的Spring Boot Starter时，又有两种方法可以实现配置
...
此配置类型为`java.util.Collection<String>`，允许开发人员指定多个包或类前缀，jimmer将用它们顾虑堆栈跟踪信息，如果某个调用帧能通过过滤，Jimmer就会认为它和用户业务相关，从而在执行前的回调参数中包含它。
