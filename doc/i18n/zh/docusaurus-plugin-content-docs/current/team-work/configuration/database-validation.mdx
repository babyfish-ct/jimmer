...
还有一个衍生问题，如果开发人员再测试环境中得到可靠的应用，那么，如何保证应用被移植到生产环境后仍然可靠？

数据库验证是一个非常重要的功能，它用于验证数据库结构和实体模型定义的一致性。
...
-   对于实体模型中定义的真外键 *(参见[真假外键](../mapping/base/foreignkey))* 而言，验证数据库中是否存在完全匹配的的外建约束
:::
...
-   实体类型必然对应于数据库的某张表，无论是候开发人员通过`@Table`显式配置表名还是由[命名策略](../mapping/base/naming-strategy)自动决定

-   基于中间表的关联属性必然对应于数据库的某张表，无论是候开发人员通过`@JoinTable`显式配置表名还是由[命名策略](../mapping/base/naming-strategy)自动决定

-   自动增长策略为`SEQUENCE`的ID必然对应于数据库的某个序列，无论是候开发人员通过`@GeneratedValue`显式配置序列名还是由[命名策略](../mapping/base/naming-strategy)自动决定
...
当然，Jimmer会优先从JDBC连接的元数据中提取字库名称，比如，从`jdbc:mysql://localhost:3306/db1`中可以提取到"db1"，优先在自动提取到的子库中查找表信息用于验证。
...
有的时候，开发人员正在开发一个新功能，它还没有完成固无法验证。开发人员自然不会去运行还未完成的功能，单期望未完成的部门不要方案验证。

对此，Jimmer提供注解`@org.babyfish.jimmer.sql.DatabaseValidationIgnore`，改注解有以下两种用法
