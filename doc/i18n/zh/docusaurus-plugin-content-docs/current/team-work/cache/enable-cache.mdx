...
:::info
对于Kotin而言，为了在覆盖`createAssociatedIdListCache`让IDE生成更好的代码，请实现`org.babyfish.jimmer.sql.kt.cache.KCacheFactory`接口。
...
-   createObjectCache：启用对象缓存
    ...
    :::note
    该实体对象是孤单的，除了当前表自身的外建所对应的多对一 *(或一对一)* 关联表现为只有id的关联对象外，没有其他关联属性。
    :::

-   createAssociatedIdCache和createAssociatedIdListCache：启用关联缓存

    createAssociatedIdCache和createAssociatedIdListCache都用于启用关联缓存。二者唯一的区别是，前者针对一对一和多对一关联，后者针对一对多和多对多关联。

    参数指定一个[关联属性](../mapping/base/association)，如果想让它支持关联缓存，就创建缓存并返回；否则，返回null。
...
-   ❸ `org.babyfish.jimmer.sql.cache.chain.SimpleBinder`是一个接口，任何不具备自动值加载值行为缓存技术都可以通过该接口适配。
...
现在，我们已经介绍了`CacheFactory`接口和多级缓存架构，但离弃用缓存还差最后一步。
