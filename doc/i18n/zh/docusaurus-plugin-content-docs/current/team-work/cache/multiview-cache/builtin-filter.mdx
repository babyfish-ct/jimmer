...
该属性默认值为`false`，表示所有受对过滤器敏感的属性 *（`BookStore.books`、`Author.books`、`BookStore.avgPrice`和`BookStore.newestBooks`）* 均不支持缓存，这是比较保守的选项。
...
-   忽略过滤行为
    ...
    -   内层结构的Hash Key，为全局过滤器的参数。
        ...
        `{}`为当前例子的对应的Hash Key，响应的缓存值为过滤器被忽略后的数据。这也是我们在单视图缓存相关文档中我们看到的数据
...
-   反转过滤行为
    ...
    -   内层结构的Hash Key，为全局过滤器的参数。
        ...
        `{"logicalDeleted":true}`为当前例子的对应的Hash Key，，表示当前缓存项的值并非依赖所有关联数据，而是依赖于已被逻辑删除的数据，这种行为和默认过滤器的逻辑刚好相反。

    :::tip
    读者可以仔细看看这些`redis-cli`命令，很容易发现，HashKey`{"logicalDeleted":false}`的数据和HashKey`{"logicalDeleted":true}`合并起来刚好就是HashKey`{}`的数据。
    :::
