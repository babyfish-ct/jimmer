...
如果既要充分利用关联缓存，又要让[对象抓取器](../../query/object-fetcher)查询返回的关联对象集合的的排序可控，可以在实体上使用静态配置制定默认排序。
:::
...
-   第二步：通过关联缓存把当前对象id转化为关联对象id
    ...
    这样，我们就得到两个`BookStore`对象各自通过一对多关联`BookStore.books`可以得到的关联对象id集合。
...
修改BOOK表的外建`STORE_ID`，Jimmer自动删除多对一关联`Book.store`和一对多`BookStore.books`的关联缓存
