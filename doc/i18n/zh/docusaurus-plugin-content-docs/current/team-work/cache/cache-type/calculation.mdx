...
:::info
如果使用Jimmer的SpringBoot Starter，触发器事件，即`EntityEvent`和`AssociationEvent`，会被作为Spring事件发送。

所以，用注解`@org.springframework.context.event.EventListener`响应Jimmer触发器的事件即可。

如果不使用Jimmer的SpringBoot Starter，采用Jimmer的触发器API响应事件
...
用户可以通过修改`BOOK`表的外建`STORE_ID`来改变`BOOK_STORE`和`BOOK`之间的关联，这必然会影响某些书店的`BookStore.avgPrice`。
...
-   ❹ 如果被修改的数据的的外键未被修改且非null
    ...
    :::tip
    这里无需考虑外建字段被修改的情况，因为前面我们已经讨论过的`onAssociationChange`会妥善处理这种情况。
    :::
