...
-   第一条SQL：查询聚合根
    ...
    即便需要以牺牲一致性为代价对其缓存，也是用户的业务问题，不是框架能抽象的统一行为。
    :::
...
-   第二条SQL：抓取多对一关联`Book.store`
    ...
    代码中的对象抓取器包含了多对一关联`Boook.store`，且期望查询关联对象的非id属性 *(当前表的外键无法表达的信息)*，所以，Jimmer会为所有聚合根对象查询关联对象。

    幸运的是，关联属性`Book.store`基于外键，外键本身就表示了父对象的id，而无需通过额外查询或[关联缓存](./association)即可得到关联对象id。
...
最终，Jimmer把两条SQL的结果拼接在一起，作为最终返回给用户的数据
```json
[
    {
        "id":1,
