...
为了充分利用kotlin的语言优势，充分发挥其开发体验，Jimmer对Java和Kotlin提供不同的API，但二者本质相同。
...
请先忽略这里生成的代码各细节的具体作用，我们可以看到大量名称包含`NoNull`或`Nullable`的类型。

:::tip
在Jimmer的Kotlin SQL DSL中，几乎所有AST类型都具备`NoNull`或`Nullable`两种实现。

这意味着，Kotlin SQL DSL具备和Kotlin语言对等的null safety特性，使SQL风格的查询代码具备完整的null saftey自检能力。
:::

让我们先看一个简单的例子

```kotlin
val bookStores = sqlClient
    ...
    .execute()
for ((name, website) in bookStores) { ❷
