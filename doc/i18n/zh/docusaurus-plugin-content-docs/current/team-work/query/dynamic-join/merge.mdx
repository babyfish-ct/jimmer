...
目前其他操作SQL的技术方案，无论ORM还是非ORM，都有存在一个问题，只考虑到了动态`where`，没有考虑`动态join`。

这里的`动态JOIN`是什么? 让我们先来看一个例子，这里用面向原生SQL的MyBatis为例
...
-   定义MyBatis的SQL映射XML
    ...
    也许你已经注意到了，❶的判断条件使用了`or`，这也不难理解。然而，这仅仅是最简单的两表连接，对于更长的表链接操作而言，动态连接的复杂度会急剧上升。

    让我们再来看一个例子，有了前面的例子作为基础，这个例子就不同任何业务耦合了。
    ...
    :::info
    动态JOIN很难处理，是当前所有其他SQL技术方案的功能缺失，无论是ORM方案还是非ORM方案，SQL字符串模板方案还是DSL方案。

    支持动态JOIN，是我创建Jimmer的最初动机。
    :::