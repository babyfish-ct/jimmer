...
更重要的是，非`id`属性当然也包括关联属性，所以此数据结构可以多层嵌套甚至递归（此处数据结构未展示多层次，但可以参考[多层关联](../initial-contact#查询复杂数据结构)和[递归关联](../initial-contact#递归查询)）。因此，这种关联也可以叫做“长关联”。

但是，并非所有时候都需要层次很深的数据结构。实际项目中，有时需要的只是一种非常简单的界面，如下
