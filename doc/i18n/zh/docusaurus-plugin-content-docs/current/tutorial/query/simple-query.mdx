...
-   不指定任何参数
    ...
    ```java
    List<Book> books = bookStoreRepository
...
```java
Page<Book> page = bookStoreRepository
    .findByName(
        null, 
        PageRequest.of(
            1, // 从0开始，1表示第二页,
...
interface BookRepository : KRepository<BookStore, Long> {
    ...
    fun findByName(
        ...
        // highlight-next-line
        fetcher: Fetcher<BookStore>? = null
    ): Page<Book>
}
```
