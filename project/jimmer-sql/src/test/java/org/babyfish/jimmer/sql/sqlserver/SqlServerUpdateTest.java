...
public class SqlServerUpdateTest extends AbstractMutationTest {
    ...
    public static void beforeAll() {
        NativeDatabases.assumeSqlserverDatabase();
    }
    ...
    /**
     * Test fluent update with JOIN-based filtering via subquery.
     * Verifies: SQL Server dialect converts JOIN conditions to subquery predicates
     * since SQL Server does not support direct JOIN syntax in UPDATE statements.
     * The generated SQL uses "where ID in (select ...)" pattern for filtered updates.
     */
    @Test
    public void fluentUpdateWithJoin() {
        ...
        executeAndExpectRowCount(
                ...
                ctx -> {
                    ctx.statement(it -> {
                        it.sql(
                                "update tb_1_ set FIRST_NAME = concat(tb_1_.FIRST_NAME, ?) " +
                                        "from AUTHOR tb_1_ " +
                                        "where tb_1_.ID in (" +
