...
> extends RecursiveRef<BaseTable3<S1, S2, S3>> {
    ...
    <T extends Table<?>> RecursiveRef4<S1, S2, S3, T> table(Class<?> tableType);
    ...
    <T extends Date> RecursiveRef4<S1, S2, S3, DateExpression<T>> date(Class<T> comparableType);

    <T extends Temporal & Comparable<T>> RecursiveRef4<S1, S2, S3, TemporalExpression<T>> temporal(Class<T> comparableType);
