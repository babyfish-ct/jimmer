...
class DraftGenerator(
    ...
    private fun TypeSpec.Builder.addFun(prop: ImmutableProp) {
        if ((prop.isAssociation(false) || prop.isList) && !prop.isGenericTarget && prop.manyToManyViewBaseProp == null && !prop.isFormula && !prop.isDiscriminator) {
            addFunction(
                FunSpec
                    .builder(prop.name)
    ...
    private fun TypeSpec.Builder.addRefFun(prop: ImmutableProp) {
        if (!prop.isAssociation(false) || prop.isGenericTarget || prop.isList || prop.isFormula || prop.isDiscriminator) {
            return
        }
        addFunction(
            FunSpec
