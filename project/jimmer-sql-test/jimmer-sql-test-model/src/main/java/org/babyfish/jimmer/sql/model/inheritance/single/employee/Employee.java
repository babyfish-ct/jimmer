 ...
 * 两个具体子类型: `FullTimeEmployee`(正式员工) 和
 * `PartTimeEmployee`(兼职员工)。鉴别器列 `EMP_TYPE` 直接定义在
 * 根接口上(单表约定,不需要 MappedSuperclass)。
 *
 * 子类型专属的标量字段(annualSalary / hourlyRate)都存放在根表
 * (STAFF_EMPLOYEE)上,只在对应子类型中有值,另一侧为 null。
 *
 * 根属性(每个子类型共享):
 *   - `supervisor` (1-level, 自关联 ManyToOne) 及其嵌套的
 *     `supervisor.department` (2-level)
 *     -> inner DTO 嵌套在外部 DTO 层
 *
 * 分支专属属性(在 `#types` 内声明):
 *   - `responsibilities` (1-level, OneToMany) 及其嵌套的
 *     `responsibilities.project` (2-level)
