...
public abstract class DtoCompiler<T extends BaseType, P extends BaseProp> {
    ...
    private class DtoTypeBuilder {
        ...
        private final Set<String> includedDtoNames = new LinkedHashSet<>();

        private final Set<String> lazyRemovedPropNames = new HashSet<>();
        ...
        private void parse(DtoParser.MacroContext macro) {
            switch (macro.name.getText()) {
                ...
                default:
                    throw new DtoAstException(
                            macro.name.getLine(),
                            "If the macro is specified, it must be \"allScalars\" or \"include\""
                    );
            }
        }

        private void allScalars(DtoParser.MacroContext macro) {
            ...
            if (!args.isEmpty()) {
                ...
                collectBaseTypes(baseType, nameBaseTypeMap, qualifiedNameBaseTypeMap);
        ...
        private void collectBaseTypes(
                T baseType,
                Map<String, List<T>> nameBaseTypeMap,
                Map<String, T> qualifiedNameBaseTypeMap
        ) {
            nameBaseTypeMap.computeIfAbsent(baseType.getName(), it -> new ArrayList<>()).add(baseType);
            qualifiedNameBaseTypeMap.put(baseType.getQualifiedName(), baseType);
            Collection<T> superTypes = getSuperTypes(baseType);
            if (superTypes != null) {
                for (T superType : superTypes) {
                    collectBaseTypes(superType, nameBaseTypeMap, qualifiedNameBaseTypeMap);
                }
            }
        }

        private void include(DtoParser.MacroContext macro) {
            List<DtoParser.QualifiedNameContext> args = macro.args;
            if (args.isEmpty()) {
                throw new DtoAstException(
                        macro.name.getLine(),
                        "The macro \"#include\" must have at least 1 argument"
                );
            }
            for (DtoParser.QualifiedNameContext qualifiedName : args) {
                if (qualifiedName.parts.size() > 1) {
                    throw new DtoAstException(
                            qualifiedName.parts.get(1).getLine(),
                            "The argument of macro \"#include\" must be simple identifier"
                    );
                }
                String dtoName = qualifiedName.parts.get(0).getText();
                includedDtoNames.add(dtoName);
            }
        }
        ...
        private void parse(DtoParser.NegativePropContext ctx) {
            if (!includedDtoNames.isEmpty()) {
                lazyRemovedPropNames.add(ctx.Identifier().getText());
                return;
            }
